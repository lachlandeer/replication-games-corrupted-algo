...
df %>%
    ggerrorplot(y = "personal_descriptive_1", x = "treatment_combined",
                desc_stat = "mean_se",
                error.plot = "errorbar",
                color = "black"
                #palette = "npg"
    ) +
    stat_summary(aes(treatment_combined, personal_descriptive_1, fill = as.factor(color_var)), alpha = 0.5, fun = mean, geom = "bar") +
    stat_compare_means(
    ...
    geom_bracket(
        xmin = "ai_honest_transparent", xmax = "ai_dishonest_transparent", y.position = 95,
        label = "***", 
    ...
    lims(y = c(-10,100)) +
    ylab("Perceived Descriptive Norms")

ggsave("out/figs/descriptive_treatment.pdf")
